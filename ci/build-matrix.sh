...
process_module() {
  ...
  for tag in "${tags[@]}"; do
    ...
    # must be vendored and not yet released
    [[ -n ${vendored["$check_tag"]:-} ]] || continue
...
build_matrix() {
  ...
  while IFS=$'\t' read -r name repo vcs; do
    ...
    process_module "${name}" "${repo}" "${vcs}" "${released_ref}" "${matrix_ref}" "${vendored_ref}"
  done < <(
